...
def route(input_features, output_features_list,  current_node):
    """
    ...
    output_features_list : list
        The list of output features for every possible candidate next hop.
        Each element is a dictionary with the following keys
        - "ACTION_NODE_ID": the name of the candidate next hop
        - "ACTION_DISTANCE": the classical routing distance from the candidate next hop to the destination
        - "ACTION_MIN_Q_LENGTH": the minimum queue length among the neighbors of the candidate next hop
        - "ACTION_MAX_Q_LENGTH": the maximum queue length among the neighbors of the candidate next hop
        - "ACTION_MEAN_Q_LENGTH": the average queue length among the neighbors of the candidate next hop
        - "ACTION_MIN_DISTANCE": the minimum classical routing from neighbors of the candidate next hop to destination
        - "ACTION_MAX_DISTANCE": the maximum classical routing from neighbors of the candidate next hop to destination
        - "ACTION_MEAN_DISTANCE": the average classical routing from neighbors of the candidate next hop to destination
